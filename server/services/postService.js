...
const createPost = (PostModel) => {
    return new Promise(async (resolve, reject) => { 
        ...
        if (existingUserResult && existingUserResult.data && existingUserResult.data.length > 0) {
            ...
            if (existingCategoryResult && existingCategoryResult.data && existingCategoryResult.data.length > 0) {
                ...
                if (existingPostResult) {
                    if (existingPostResult.data && existingPostResult.data.length > 0) {
                        ...
                        connection.query(query, [PostModel.category_id, PostModel.user_id, PostModel.title, PostModel.body, PostModel.image, formattedDateTimeNow], (error, results) => {
                            if (error) {
                                response.success = false;
...
const updatePost = (PostModel) => {
    return new Promise(async (resolve, reject) => {
        ...
        connection.query(query, [PostModel.title, PostModel.body, PostModel.image, PostModel.id], (error, results) => {
            if (error) {
                console.log(error);
                response.success = false;
                response.message = `Error querying the database: ${error}`;
